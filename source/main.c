                ...
                if(g->rows[gy]&(1u<<(4-gx)))
                    for(int sy=0;sy<scale;sy++)
                        for(int sx=0;sx<scale;sx++)
                            put(f,x+gx*scale+sx,y+gy*scale+sy,c);
        x+=6*scale;
    }
}
...
static bool parse_hex(const char *s,Color *out) {
    ...
    u8 d[6];
    for(int i=0;i<6;i++) if(!hd(s[i],&d[i])) return false;
    out->r=(u8)((d[0]<<4)|d[1]);
    out->g=(u8)((d[2]<<4)|d[3]);
    out->b=(u8)((d[4]<<4)|d[5]);
    return true;
...
static void keyboard(void) {
    SwkbdState st;
    char in[8]={0};
    swkbdInit(&st,SWKBD_TYPE_WESTERN,2,7);
    swkbdSetHintText(&st,"#RRGGBB");
    swkbdSetValidation(&st,SWKBD_NOTEMPTY_NOTBLANK,0,0);
    if(swkbdInputText(&st,in,sizeof(in))==SWKBD_BUTTON_CONFIRM) {
        Color c;
        if(parse_hex(in,&c)) custom_color=c;
    }
}

static int slider_x(int v) { return 62+(v*216)/255; }
static int slider_v(int x) {
    if(x<=62) return 0;
    if(x>=278) return 255;
    return ((x-62)*255)/216;
}
...
static void slider(const Frame *f,int y,char lab,int v,Color fc,bool p) {
    Color tr={58,63,74}, wh={245,247,250};
    text(f,25,y-5,(char[]){lab,0},2,wh);
    roundrect(f,62,y,216,12,6,tr);
    roundrect(f,62,y,slider_x(v)-62,12,6,fc);
    circle(f,slider_x(v),y+6,p?9:7,p?dark(wh):wh);
}

static void draw_menu(const Frame *f) {
    Color panel={35,39,48},inactive={53,58,70},sel={58,102,190};
    Color wh={245,247,250},close={125,48,55},ap={48,119,72};
    clear_frame(f,bottom_color);
    roundrect(f,4,4,312,232,16,panel);

    Color a=tab==0?sel:inactive,b=tab==1?sel:inactive;
    if(pressed_button==BTN_TAB_COMMON) a=dark(a);
    if(pressed_button==BTN_TAB_CUSTOM) b=dark(b);
    roundrect(f,12,14,138,30,8,a);
    roundrect(f,170,14,138,30,8,b);
    centered(f,81,23,"COMMON",2,wh);
    centered(f,239,23,"CUSTOM",2,wh);

    if(tab==0) {
        for(int i=0;i<COMMON_COUNT;i++) {
            int col=i%4,row=i/4;
            Color c=common_colors[i];
            if(pressed_button==BTN_COLOR_BASE+i) c=dark(c);
            roundrect(f,14+col*73,56+row*41,65,34,8,c);
        }
    } else {
        ...
        Color ab=ap,hb=inactive;
        if(pressed_button==BTN_APPLY) ab=dark(ab);
        if(pressed_button==BTN_HEX) hb=dark(hb);
        roundrect(f,18,166,137,28,8,ab);
        roundrect(f,165,166,137,28,8,hb);
        centered(f,86,175,"APPLY",2,wh);
        centered(f,234,175,"CUSTOM",2,wh);
    }

    Color cb=close;
    if(pressed_button==BTN_CLOSE) cb=dark(cb);
    roundrect(f,12,204,296,24,8,cb);
    centered(f,160,211,"CLOSE MENU",2,wh);
}

static void draw_credits(const Frame *f) {
    Color panel={35,39,48},wh={245,247,250};
    Color close={70,90,125},exitc={125,48,55};
    clear_frame(f,bottom_color);
    roundrect(f,4,4,312,232,16,panel);

    centered(f,160,20,"COLOR 3DS",3,wh);

    /* Deliberately spaced out so the information is easy to read. */
    centered(f,160,52,"A SIMPLE SCREEN COLOR",2,wh);
    centered(f,160,66,"CHANGER FOR NINTENDO 3DS",2,wh);

    centered(f,160,96,"MADE WITH AI",2,wh);

    centered(f,160,124,"THIS APP CHANGES COLORS",2,wh);
    centered(f,160,138,"INSIDE THE APP ONLY",2,wh);

    if(pressed_button==BTN_CREDITS_CLOSE) close=dark(close);
    if(pressed_button==BTN_CREDITS_EXIT) exitc=dark(exitc);

    roundrect(f,12,174,296,24,8,close);
    roundrect(f,12,204,296,24,8,exitc);
    centered(f,160,181,"CLOSE CREDITS",2,wh);
    centered(f,160,211,"CLOSE APP",2,wh);
}

static int hit(int x,int y) {
    if(credits_open) {
        if(y>=174&&y<198&&x>=12&&x<308) return BTN_CREDITS_CLOSE;
        if(y>=204&&y<228&&x>=12&&x<308) return BTN_CREDITS_EXIT;
        return NO_BUTTON;
    }

    if(y>=14&&y<44) {
        if(x>=12&&x<150) return BTN_TAB_COMMON;
        if(x>=170&&x<308) return BTN_TAB_CUSTOM;
    }

    if(tab==0&&x>=14&&x<306&&y>=56&&y<179) {
        int col=(x-14)/73,row=(y-56)/41;
        int lx=(x-14)%73,ly=(y-56)%41;
        if(col<4&&row<3&&lx<65&&ly<34) {
            int i=row*4+col;
    ...
    if(tab==1) {
        if(y>=59&&y<89) return BTN_SLIDER_R;
        if(y>=95&&y<125) return BTN_SLIDER_G;
        if(y>=131&&y<161) return BTN_SLIDER_B;
        if(y>=166&&y<194) {
            if(x<155) return BTN_APPLY;
            if(x>=165) return BTN_HEX;
        }
    }

    if(y>=204&&y<228&&x>=12&&x<308) return BTN_CLOSE;
    return NO_BUTTON;
}

static void activate(int b,Target t) {
    if(b==BTN_CREDITS_CLOSE) {
        credits_open=false;
    } else if(b==BTN_CREDITS_EXIT) {
        /* Actually request application termination. */
        credits_open=false;
        menu_open=false;
        app_exit_requested=true;
    } else if(b==BTN_TAB_COMMON) {
        tab=0;
    } else if(b==BTN_TAB_CUSTOM) {
        tab=1;
    } else if(b==BTN_CLOSE) {
        menu_open=false;
    } else if(b==BTN_APPLY) {
        apply_color(custom_color,t);
    } else if(b==BTN_HEX) {
        keyboard();
    } else if(b>=BTN_COLOR_BASE&&b<BTN_COLOR_BASE+COMMON_COUNT) {
        apply_color(common_colors[b-BTN_COLOR_BASE],t);
    }
}

static void touch_update(u32 down,u32 held) {
    static int touch_button=NO_BUTTON;
    static bool touching=false;
    touchPosition p;

    if(down&KEY_TOUCH) {
        hidTouchRead(&p);
        touch_button=hit(p.px,p.py);
        pressed_button=touch_button;
        pressed_target=target(held);
        touching=true;
        return;
    }
    ...
    if(held&KEY_TOUCH) {
        hidTouchRead(&p);
        if(hit(p.px,p.py)!=touch_button) {
            touch_button=NO_BUTTON;
...
static void update_slider(u32 held) {
    ...
    hidTouchRead(&p);
    int v=slider_v(p.px);
    if(pressed_button==BTN_SLIDER_R) custom_color.r=v;
    else if(pressed_button==BTN_SLIDER_G) custom_color.g=v;
    else custom_color.b=v;
}
...
int main(void) {
    ...
    while(aptMainLoop() && !app_exit_requested) {
        ...
        /* Hold SELECT for one second anywhere to open credits. */
        if(kHeld&KEY_SELECT) {
            if(!select_held) {
                select_tick=svcGetSystemTick();
                select_held=true;
            } else if(!credits_open) {
                u64 elapsed=svcGetSystemTick()-select_tick;
        ...
        /* Hold START for one second to open the color menu. */
        if(kHeld&KEY_START) {
            if(!start_held) {
                start_tick=svcGetSystemTick();
                start_held=true;
            } else if(!menu_open && !credits_open) {
        ...
        if(kDown&KEY_B) {
            if(credits_open) credits_open=false;
            else if(menu_open) menu_open=false;
            else break;
        }

        if(menu_open && !credits_open) {
            touch_update(kDown,kHeld);
            update_slider(kHeld);
        }
