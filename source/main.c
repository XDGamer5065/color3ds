...
static bool menu_open=false,credits_open=false,start_held=false,select_held=false;
...
static void draw_credits(const Frame*f){Color panel={35,39,48},wh={245,247,250},close={125,48,55},exitc={125,48,55};clear_frame(f,bottom_color);roundrect(f,4,4,312,232,16,panel);centered(f,160,25,"COLOR 3DS",3,wh);centered(f,160,58,"A SIMPLE SCREEN COLOR",2,wh);centered(f,160,72,"CHANGER FOR NINTENDO 3DS",2,wh);centered(f,160,104,"MADE WITH AI",2,wh);centered(f,160,119,"THIS APP CHANGES COLORS",2,wh);centered(f,160,133,"INSIDE THE APP ONLY",2,wh);if(pressed_button==BTN_CREDITS_CLOSE)close=dark(close);if(pressed_button==BTN_CREDITS_EXIT)exitc=dark(exitc);roundrect(f,12,174,296,24,8,close);roundrect(f,12,204,296,24,8,exitc);centered(f,160,181,"CLOSE CREDITS",2,wh);centered(f,160,211,"CLOSE APP",2,wh);}
static int hit(int x,int y){if(credits_open){if(y>=174&&y<198&&x>=12&&x<308)return BTN_CREDITS_CLOSE;if(y>=204&&y<228&&x>=12&&x<308)return BTN_CREDITS_EXIT;return NO_BUTTON;}if(y>=14&&y<44){if(x>=12&&x<150)return BTN_TAB_COMMON;if(x>=170&&x<308)return BTN_TAB_CUSTOM;}if(tab==0&&x>=14&&x<306&&y>=56&&y<179){int col=(x-14)/73,row=(y-56)/41,lx=(x-14)%73,ly=(y-56)%41;if(col<4&&row<3&&lx<65&&ly<34){int i=row*4+col;if(i<COMMON_COUNT)return BTN_COLOR_BASE+i;}}if(tab==1){if(y>=59&&y<89)return BTN_SLIDER_R;if(y>=95&&y<125)return BTN_SLIDER_G;if(y>=131&&y<161)return BTN_SLIDER_B;if(y>=166&&y<194){if(x<155)return BTN_APPLY;if(x>=165)return BTN_HEX;}}if(y>=204&&y<228&&x>=12&&x<308)return BTN_CLOSE;return NO_BUTTON;}
static void activate(int b,Target t){if(b==BTN_CREDITS_CLOSE)credits_open=false;else if(b==BTN_CREDITS_EXIT)credits_open=false;else if(b==BTN_TAB_COMMON)tab=0;else if(b==BTN_TAB_CUSTOM)tab=1;else if(b==BTN_CLOSE)menu_open=false;else if(b==BTN_APPLY)apply_color(custom_color,t);else if(b==BTN_HEX)keyboard();else if(b>=BTN_COLOR_BASE&&b<BTN_COLOR_BASE+COMMON_COUNT)apply_color(common_colors[b-BTN_COLOR_BASE],t);}
...
static void update_slider(u32 held){if(!(held&KEY_TOUCH)||pressed_button<BTN_SLIDER_R||pressed_button>BTN_SLIDER_B)return;touchPosition p;hidTouchRead(&p);int v=slider_v(p.px);if(pressed_button==BTN_SLIDER_R)custom_color.r=v;else if(pressed_button==BTN_SLIDER_G)custom_color.g=v;else custom_color.b=v;}
int main(void){gfxInitDefault();Color green={0,255,0};top_color=bottom_color=custom_color=green;while(aptMainLoop()){hidScanInput();u32 down=hidKeysDown(),held=hidKeysHeld();if(held&KEY_START){if(!start_held){start_held=true;start_tick=svcGetSystemTick();}else if(!menu_open&&!credits_open&&svcGetSystemTick()-start_tick>=HOLD_TICKS){menu_open=true;tab=0;}}else start_held=false;if(held&KEY_SELECT){if(!select_held){select_held=true;select_tick=svcGetSystemTick();}else if(!credits_open&&svcGetSystemTick()-select_tick>=HOLD_TICKS){credits_open=true;menu_open=false;pressed_button=NO_BUTTON;}}else select_held=false;if(credits_open||menu_open){touch_update(down,held);if(menu_open)update_slider(held);}Frame top=frame_get(GFX_TOP),bot=frame_get(GFX_BOTTOM);clear_frame(&top,top_color);clear_frame(&bot,bottom_color);if(credits_open)draw_credits(&bot);else if(menu_open)draw_menu(&bot);gfxFlushBuffers();gspWaitForVBlank();gfxSwapBuffers();}gfxExit();return 0;}
